...
# Parse choices from positional argument (e.g. ./start-dev.sh 2111)
# Digits: 1=PROJECT_CHOICE, 2=GAME_CHOICE, 3=MOBILE_CHOICE, 4=ANDROID_CHOICE
if [ -n "$1" ]; then
    ARG="$1"
    [ -z "$PROJECT_CHOICE" ] && PROJECT_CHOICE="${ARG:0:1}"
    [ -z "$GAME_CHOICE" ]    && GAME_CHOICE="${ARG:1:1}"
    [ -z "$MOBILE_CHOICE" ]  && MOBILE_CHOICE="${ARG:2:1}"
    [ -z "$ANDROID_CHOICE" ] && ANDROID_CHOICE="${ARG:3:1}"
fi

# Ask which project to run (unless already set)
if [ -z "$PROJECT_CHOICE" ]; then
    echo -e "${YELLOW}Какой проект запускать?${NC}"
...
# Update config.ts with backend URL (dev mode - no Basic Auth)
echo -e "${YELLOW}Updating ${PROJECT_APP} configuration...${NC}"
...
# Update .env with active game
cat > ${PROJECT_APP}/.env <<EOF
# Auto-generated by start-dev.sh on $(date)
ACTIVE_GAME=${ACTIVE_GAME}
BACKEND_URL=${BACKEND_URL}
EOF
...
if [ -z "$MOBILE_CHOICE" ]; then
    echo -e "\n${YELLOW}Запустить мобильное приложение (${PROJECT_APP})?${NC}"
...
if [ -n "$MOBILE_CMD" ]; then
    ...
    tmux send-keys -t $SESSION_NAME "cd '$PROJECT_DIR/${PROJECT_APP}'" C-m
    tmux send-keys -t $SESSION_NAME "clear" C-m
    tmux send-keys -t $SESSION_NAME "echo -e '${BLUE}=== Mobile (React Native / ${PROJECT_APP}) ===${NC}'" C-m
